...
def main():
    ...
    argparser.add_argument('--label', type=str, default='all',
                           help='Specify the label of output. Default is `all`.')
    argparser.add_argument(
        '--stat', help='Print extracted features and ratio to I/O if True', action='store_true')
    argparser.add_argument(
    ...
    argparser.add_argument(
        '--fullstat', help='Print judgement to I/O if True', action='store_true')
    argparser.add_argument(
        '--fast', help='Fast mode if True', action='store_true')
    ...
    with open(args.input, encoding='utf-8') as f:
        for line in f:
            l = line.strip()
            judgement, canto, swc = judge(
                l, split_seg=args.split, get_quote=args.quotes, print_stat=args.fullstat, fast_mode=args.fast)
            _line = judgement
            if args.stat:
